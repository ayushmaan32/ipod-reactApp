...
class Navbar extends React.Component {
    ...
    render() {
        ...
        const { noty, notifyText, playing } = this.props;
        ...
        return (
            <div className="Navbar">
                ...
                {noty === true && <h5 className="notification">
                    {notifyText}</h5>}
                {noty===false && <h3 className="time">
                    {time}</h3>}
                {<div className="right-container-nav">
                 {playing ? <h5 className="play-pause-nav">
                 <i className="fas fa-play"></i> </h5>:  <h5 className="play-pause-nav"><i className="fas fa-pause"></i> 
                 </h5>} 
                 <img className="battery" src={BatryImg} alt="Battery" />
                 </div>
                   }
